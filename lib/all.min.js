...
if(action=="audioplayer-volume" && _midi[file].state=="") return false; if(!_midi[file].timidity) { var dir=window.location.href.substr(0,window.location.href.lastIndexOf("/")); var base=$("base").attr("href").split("/")[0]; _midi[file].timidity=require('timidity'); _midi[file].player=new _midi[file].timidity(dir+"/"+base+"/"+"lib/timidity");
//~ _midi[file].player.on("error", (err) => {
//~ console.log("error");
//~ console.log(err);
//~ })
_midi[file].player.on("timeupdate", () => { if(_midi[file].player.duration==1) return; var currenttime=_midi[file].player.currentTime; var duration=_midi[file].player.duration; $(".audioplayer-time-current",_midi[file].div).html(_sec2time(currenttime)); $(".audioplayer-time-duration",_midi[file].div).html(_sec2time(duration)); $(".audioplayer-bar-played",_midi[file].div).width(parseInt(currenttime*100/duration)+"%"); });
//~ _midi[file].player.on("unstarted", () => {
//~ console.log("unstarted");
//~ });
_midi[file].player.on("ended", () => { _midi[file].player.seek(0); _midi[file].state="stop"; $(_midi[file].div).removeClass("audioplayer-playing"); $(_midi[file].div).addClass("audioplayer-stopped"); $(".audioplayer-playpause",_midi[file].div).attr("title","Play"); $(".audioplayer-playpause a",_midi[file].div).html("Play"); });
...
//~ })
_midi[file].player.load(dir+"/"+base+"/"+file); $(".audioplayer-bar-loaded",_midi[file].div).width("100%"); } if(action=="audioplayer-playpause") { if(_midi[file].state!="play") { _midi[file].player.play(); _midi[file].state="play"; $(_midi[file].div).removeClass("audioplayer-stopped"); $(_midi[file].div).addClass("audioplayer-playing"); $(".audioplayer-playpause",_midi[file].div).attr("title","Pause"); $(".audioplayer-playpause a",_midi[file].div).html("Pause"); } else { _midi[file].player.pause(); _midi[file].state="pause"; $(_midi[file].div).removeClass("audioplayer-playing"); $(_midi[file].div).addClass("audioplayer-stopped"); $(".audioplayer-playpause",_midi[file].div).attr("title","Play"); $(".audioplayer-playpause a",_midi[file].div).html("Play"); } } if(action=="audioplayer-volume") { if(_midi[file].state=="play") _midi[file].player.pause(); _midi[file].player.seek(0); _midi[file].state="stop"; $(_midi[file].div).removeClass("audioplayer-playing"); $(_midi[file].div).addClass("audioplayer-stopped"); $(".audioplayer-playpause",_midi[file].div).attr("title","Play"); $(".audioplayer-playpause a",_midi[file].div).html("Play"); } if(action=="audioplayer-bar") { var x=('ontouchstart' in window)?e.originalEvent.touches[0].pageX:e.pageX; var duration=_midi[file].player.duration; var left=$(".audioplayer-bar-loaded",_midi[file].div).offset().left; var width=$(".audioplayer-bar-loaded",_midi[file].div).width(); _midi[file].player.seek(parseFloat(duration*(x-left)/width)); } return false; }); }