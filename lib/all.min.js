...
$("#files tbody").html("");
// MUNTAR NOVA TAULA
for(var key in data.songs[hash].files) { var val=data.songs[hash].files[key]; var type=val.last.split('.').pop(); var label=data.labels[type]; if(["pdf"].includes(type)) { var num=str_replace([type,".","-"],"",val.last); if(num=="") num="a4"; if(["a4","a5","a6"].includes(num)) label+=" "+data.labels["din"+num]; } if(["midi","mp3"].includes(type)) { var num=str_replace([type,".","-"],"",val.last); if(num=="") label+=" "+data.labels["pista0"]; else label+=" "+str_replace("__NUM__",num,data.labels["pista1"]); } var file="<a href='files/"+hash+"/"+hash+val.last+"'>"+hash+val.last+"</a>"; var play=""; if(type=="mp3") { play=[ "<audio preload='none' src='files/"+hash+"/"+hash+val.last+"' controls></audio>", ].join(" "); } var size=__getmail_gethumansize(val.size); $("#files tbody").append(str_replace(["__TYPE__","__FILE__","__SIZE__","__PLAY__"],[label,file,size,play],data.template[4])); }
// CREAR AUDIOPLAYER
$("audio").audioPlayer();
// MOSTRAR TAULA
