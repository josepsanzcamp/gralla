...
$("#menu .collapse.show").each(function () { new bootstrap.Collapse(this); }); $("#menu a.active").removeClass("active");
// AMAGAR CERCADOR I MENU
$("#result").addClass("d-none"); $("#sidebarMenu.show").each(function () { new bootstrap.Collapse(this); });
// CHECK
if (hash.substr(0,7) != "search=") {
// ACTIVAR LA NOVA SELECCIO
var hash0 = hash.split("-")[0]; $("#c_" + hash0).each(function () { new bootstrap.Collapse(this); }); $("#c_" + hash0 + " a[href='#" + hash + "']").addClass("active");
// ACTUALITZAR TITOLS
...
$(document).on("click","a",function () { var hash1 = decodeURI(window.location.hash); var hash2 = $(this).attr("href"); if (hash1 != hash2) { return; } $("#sidebarMenu.show").each(function () { new bootstrap.Collapse(this); }); $("#result").addClass("d-none"); $("#files").removeClass("d-none"); $("#pdf").removeClass("d-none"); });
...
function _pdf(hash) { $("#pdf").html("<div id='viewerContainer'><div id='viewer' class='pdfViewer'></div></div>"); pdfjsLib.GlobalWorkerOptions.workerSrc = "lib/pdfjs/pdf.worker.min.js"; var base = $("base").attr("href").split("/")[0]; pdfjsLib.getDocument(base + "/files/" + hash + "/" + hash + ".pdf").promise.then(function (pdfDocument) { if (!pdfDocument.numPages) { return; } var container = document.getElementById("viewerContainer"); var eventBus = new pdfjsViewer.EventBus(); _pdfViewer = new pdfjsViewer.PDFViewer({ container:container, eventBus:eventBus, }); eventBus.on("pagesinit",function () { _pdfViewer.currentScaleValue = "page-width"; }); eventBus.on("annotationlayerrendered",function () { $("a",container).each(function () { $(this).attr("target","_blank"); }); }); _pdfViewer.removePageBorders = true; _pdfViewer.setDocument(pdfDocument); $(container).css("position","relative"); $("#pdf").removeClass("d-none"); },function (message,exception) { console.log(message); }); }
...
$("#sidebarMenu.show").each(function () { new bootstrap.Collapse(this); }); $("#result").removeClass("d-none"); $("#files").addClass("d-none"); $("#pdf").addClass("d-none"); }
