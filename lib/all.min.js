...
$("#files tbody").html("");
// MUNTAR NOVA TAULA
for(var key in data.songs[hash].files) { var val=data.songs[hash].files[key]; var type=val.last.split('.').pop(); var label=data.labels[type]; if(type=="pdf") { var text=str_replace([type,".","-"],"",val.last).toUpperCase(); if(text!="") label+=" "+str_replace("__TEXT__",text,data.labels["papersize"]); } if(["midi","mp3"].includes(type)) { var num=str_replace([type,".","-"],"",val.last); if(num!="") label+=" "+str_replace("__NUM__",num,data.labels["pista"]); } var file="<a href='files/"+hash+"/"+hash+val.last+"'>"+hash+val.last+"</a>"; var play=""; if(type=="mp3") { play="<audio preload='none' src='files/"+hash+"/"+hash+val.last+"' controls></audio>"; } if(type=="midi") { play="<midi file='files/"+hash+"/"+hash+val.last+"'></midi>"; } var size=__getmail_gethumansize(val.size); $("#files tbody").append(str_replace(["__TYPE__","__FILE__","__SIZE__","__PLAY__"],[label,file,size,play],data.template[4])); }
// CREAR AUDIOPLAYER
