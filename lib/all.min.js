...
/**
* @licstart The following is the entire license notice for the * JavaScript code in this page * * Copyright 2023 Mozilla Foundation * * Licensed under the Apache License, Version 2.0 (the "License"); * you may not use this file except in compliance with the License. * You may obtain a copy of the License at *
...
/**
* Minified by jsDelivr using Terser v5.15.1. * Original file: /npm/pdfjs-dist@3.5.141/web/pdf_viewer.js *
...
// VARIABLES GLOBALS
var _random_hash = ""; var _pdfViewer = null; var _midi = {}; $(function () {
// PINTAR MENU
_menu();
// PROGRAMAR HASHCHANGE
$(window).on("hashchange",function () {
// AGAFAR EL HASH ACTUAL
var hash = decodeURI(window.location.hash); if (hash.substr(0,1) == "#") { hash = hash.substr(1); } if (hash.substr(0,7) != "search=" && !data.songs.hasOwnProperty(hash)) {
// SINO, MIRAR SI HAY PARAMETRO HASH
if (typeof URLSearchParams == "function") { var params = new URLSearchParams(window.location.search); if (params.has("hash")) { hash = decodeURI(params.get("hash")); } } if (hash.substr(0,7) != "search=" && !data.songs.hasOwnProperty(hash)) {
// SINO, MIRAR EL NOM DEL FITXER
hash = decodeURI(window.location.href.split("/").pop().split(".")[0]); if (hash.substr(0,7) != "search=" && !data.songs.hasOwnProperty(hash)) {
// SINO, FER SERVIR COOKIE
hash = decodeURI($.cookie("hash")); if (typeof(hash) == "undefined") { hash = ""; } if (hash.substr(0,7) != "search=" && !data.songs.hasOwnProperty(hash)) {
...
// PINTAR MENU
for (var key in data.cats) { var val = data.cats[key]; $("#menu").append(str_replace(["__TITLE__","__HEADING__","__COLLAPSE__"],[val.name,"h_" + key,"c_" + key],data.template[0]) + data.template[2]); for (var key2 in val.array) { var val2 = val.array[key2]; $("#c_" + key + " ul").append(str_replace(["__TITLE__","__LINK__"],[data.songs[val2].name,"#" + val2],data.template[1])); } }
// CALCULAR RANDOM HASH
var keys = Object.keys(data.cats); var key = keys[keys.length * Math.random() << 0]; var val = data.cats[key]; var key2 = val.array.length * Math.random() << 0; var val2 = val.array[key2]; _random_hash = val2;
// MOSTRAR MENU
