...
function _pdf(hash) { $("#pdf").html("<div id='viewerContainer'><div id='viewer' class='pdfViewer'></div></div>"); pdfjsLib.GlobalWorkerOptions.workerSrc = "lib/pdfjs/pdf.worker.min.js"; var base = $("base").attr("href").split("/")[0]; pdfjsLib.getDocument(base + "/files/" + hash + "/" + hash + ".pdf").promise.then(function (pdfDocument) { if (!pdfDocument.numPages) { return; } var container = document.getElementById("viewerContainer"); var eventBus = new pdfjsViewer.EventBus(); _pdfViewer = new pdfjsViewer.PDFViewer({ container:container, eventBus:eventBus, }); var fn1 = function () { _pdfViewer.currentScaleValue = "page-width"; }; var fn2 = function () { $("a",container).each(function () { $(this).attr("target","_blank"); }); }; eventBus.on("pagesinit",fn1); eventBus.on("annotationlayerrendered",fn2); _pdfViewer.removePageBorders = true; _pdfViewer.setDocument(pdfDocument); $(container).css("position","relative"); $("#pdf").removeClass("d-none"); },function (message,exception) { console.log(message); console.log(exception); }); }
