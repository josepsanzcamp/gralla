...
$(function() {
	...
	for(var key in data.cats) {
		...
		$("#menu > div:last").before(str_replace(["__TITLE__","__HEADING__","__COLLAPSE__"],[val.name,"h_"+key,"c_"+key],data.template[0])+data.template[2]);
	...
	$(".collapse[id^='c_']").on("show.bs.collapse", function() {
		...
		$(".collapse[id^='c_']").each(function() {
			if(_this==this) {
				var height0=$("nav:first").outerHeight(); // BARRA BLAVA
				var height1=$("#menu .card-header:eq(0)").outerHeight(); // CAIXA CERCADOR
				var height2=$("#menu .card-header:eq(1)").outerHeight(); // RESTA BOTONS
...
function _files(hash) {
	...
	for(var key in data.songs[hash].files) {
		...
		$("#files tbody").append(str_replace(["__TYPE__","__FILE__","__SIZE__","__PLAY__"],[label,file,val.size,play],data.template[4]));
	}
	// MOSTRAR TAULA
