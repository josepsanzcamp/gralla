...
$(function() {
	...
	$(".collapse[id^='c_']").on("show.bs.collapse", function() {
		...
		$(".collapse[id^='c_']").each(function() {
			if(_this==this) {
				var height0=$("nav:first").outerHeight();
				var height1=$("#menu .card-header:eq(0)").outerHeight();
				var height2=$("#menu .card-header:eq(1)").outerHeight();
...
function _files(hash) {
	...
	for(var key in data.songs[hash].files) {
		...
		if(type=="mp3") {
			play=[
				"<audio src='files/"+hash+val.last+"'></audio>",
