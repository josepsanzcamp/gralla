...
function _files(hash) {
	...
	for(var key in data.songs[hash].files) {
		...
		if(type=="pdf") {
			...
			if(num=="") num="a4";
			if(["a4","a5","a6"].includes(num)) label+=" "+data.labels["din"+num];
		}
		if(["midi","mp3"].includes(type)) {
			var num=str_replace([type,".","-"],"",val.last);
			if(num=="") label+=" "+data.labels["pista0"];
			else label+=" "+str_replace("__NUM__",num,data.labels["pista1"]);
		}
...
function _pdf(hash,last=".pdf") {
	pdfjsLib.GlobalWorkerOptions.workerSrc="lib/pdfjs/pdf.worker.min.js";
	var base=$("base").attr("href").split("/")[0];
	pdfjsLib.getDocument(base+"/files/"+hash+"/"+hash+last).promise.then(function(pdfDocument) {
