...
(function () {
    ...
    // PER DETECTAR CANVI DE MODE AMB EL MENU OBERT DEL MODE MOBIL
    window.addEventListener('layoutchange', e => {
        if (!document.getElementById("sidebarMenu").classList.contains("show")) {
            return;
        }
        if (e.detail.mode == 'desktop') {
            document.getElementById("result").classList.add("d-none");
            document.getElementById("files").classList.remove("d-none");
            document.getElementById("pdf").classList.remove("d-none");
        }
        if (e.detail.mode == 'mobile') {
            document.getElementById("result").classList.add("d-none");
            document.getElementById("files").classList.add("d-none");
            document.getElementById("pdf").classList.add("d-none");
        }
    ...
    // PER GENERAR UN EVENT QUAN SALTA ENTRE MODES DESKTOP I MOBILE
    var window_match_media_size = window.matchMedia('(min-width: 768px)');
    var fire_event = e => window.dispatchEvent(new CustomEvent('layoutchange', { detail: { mode: e.matches ? 'desktop' : 'mobile' }}));
    fire_event(window_match_media_size);
    window_match_media_size.addEventListener('change', fire_event);
...
function _pdf(hash)
{
    ...
    for (var key in data.songs[hash].pages) {
        ...
        var img = "files/" + hash + "/" + hash + val.last;
        var width = val.width;
        var height = val.height;
        var alt = data.songs[hash].name;
        document.getElementById("pdf").append(saltos.html('<img src="' + img + '" class="img-fluid w-100 mb-3 shadow-sm border ' + colors[color % 2] + '" loading="lazy" width="' + width + '" height="' + height + '" alt="' + alt + '"/>'));
        color++;
    }
    document.getElementById("pdf").classList.remove("d-none");
}
...
function _search()
{
    ...
    if (oldtext.substr(0,1) == '^') {
        oldtext = oldtext.substr(1);
        var len = oldtext.length;
        // CERCAR PER PREFIX DEL HASH
        var result = {};
        for (var key in data.songs) {
            if (key.substr(0, len) == oldtext) {
...
{
    var div = saltos.html('\
    <div class="audioplayer audioplayer-stopped midiplayer rounded-pill">\
        <div class="audioplayer-playpause rounded-start-pill" title="Play"><span>Play</span></div>\
        ...
        <div class="audioplayer-volume rounded-end-pill" title="Stop"><span>Stop</span>\
    </div>\
    ');
...
function _audiofy(obj)
{
    ...
    var fn = function (e) {
        ...
        if (action.includes("audioplayer-playpause")) {
            if (audio.paused) {
                audio.play();
        ...
        if (action.includes("audioplayer-volume")) {
            if (!audio.paused) {
                audio.pause();
            }
            audio.currentTime = 0;
        ...
        if (action.includes("audioplayer-bar")) {
