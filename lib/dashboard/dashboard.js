...
function _files(hash) {
    ...
    for(var key in data.songs[hash].files) {
        ...
        if(type=="midi") {
            play="<midi file='files/"+hash+"/"+hash+val.last+"'></midi>";
        }
...
function _midify(obj) {
    ...
    $(".audioplayer-playpause,.audioplayer-volume,.audioplayer-bar",div).on("click",function(e) {
        var div=$(this).parent();
        var file=$(div).attr("file");
        var action=$(this).attr("class");

        // TRUC PER EVITAR CARREGAR-HO TOT QUAN NO CAL
        ...
        if(!_midi[file].timidity) {
            ...
            //~ _midi[file].player.on("error", (err) => {
                //~ console.log("error");
                //~ console.log(err);
            //~ })
            ...
            _midi[file].player.on("timeupdate", () => {
                ...
                var duration=_midi[file].player.duration;
                        $(".audioplayer-time-current",_midi[file].div).html(_sec2time(currenttime));
                        $(".audioplayer-time-duration",_midi[file].div).html(_sec2time(duration));
                $(".audioplayer-bar-played",_midi[file].div).width(parseInt(currenttime*100/duration)+"%");
            });

            //~ _midi[file].player.on("unstarted", () => {
            ...
            _midi[file].player.on("ended", () => {
                ...
                $(_midi[file].div).removeClass("audioplayer-playing");
                $(_midi[file].div).addClass("audioplayer-stopped");
                $(".audioplayer-playpause",_midi[file].div).attr("title","Play");
                $(".audioplayer-playpause a",_midi[file].div).html("Play");
            });

            //~ _midi[file].player.on("playing", () => {
                //~ console.log("playing");
            //~ });

            //~ _midi[file].player.on("paused", () => {
                //~ console.log("paused");
            //~ })

            //~ _midi[file].player.on("buffering", () => {
                //~ console.log("buffering");
            //~ })

            _midi[file].player.load(dir+"/"+base+"/"+file);
            $(".audioplayer-bar-loaded",_midi[file].div).width("100%");
        }
        ...
        if(action=="audioplayer-playpause") {
            if(_midi[file].state!="play") {
                ...
                $(_midi[file].div).removeClass("audioplayer-stopped");
                $(_midi[file].div).addClass("audioplayer-playing");
                $(".audioplayer-playpause",_midi[file].div).attr("title","Pause");
                $(".audioplayer-playpause a",_midi[file].div).html("Pause");
            } else {
        ...
        if(action=="audioplayer-bar") {
            ...
            var duration=_midi[file].player.duration;
            var left=$(".audioplayer-bar-loaded",_midi[file].div).offset().left;
            var width=$(".audioplayer-bar-loaded",_midi[file].div).width();
