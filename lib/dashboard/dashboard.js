...
$(function() {
	...
	// ARRECLAR MIDA PRIMER ITEM
	$("#accordionExample .card-header:first").height($("#accordionExample .card-header:last").height());

	// PINTAR MENU
	var first_hash="";
	for(var key in data.cats) {
		var val=data.cats[key];
		$("#accordionExample > div:last").before(str_replace(["__TITLE__","__HEADING__","__COLLAPSE__"],[val.name,"h_"+key,"c_"+key],data.template[0])+data.template[2]);
