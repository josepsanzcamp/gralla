...
$(function() {
	...
	$(window).on("hashchange",function() {
		...
		if($(window).width()>=1200) {
			...
			$(".navbar-brand").attr("todo",data.title);
		...
		for(var key in data.songs[hash].files) {
			...
			if(val.type=="mp3") {
				play=[
					...
					`<a href="javascript:void(0);" class="play" onclick="_play(this);">Play</a>`,
					`<a href="javascript:void(0);" class="pause d-none" onclick="_pause(this);">Pause</a>`,
					`<a href="javascript:void(0);" class="stop d-none" onclick="_stop(this);">Stop</a>`,
				].join(" ");
			}
			$("#qwerty tbody").append(str_replace(["__TYPE__","__FILE__","__SIZE__","__PLAY__"],[val.label,file,val.size,play],data.template[3]));
		}
		// PINTAR PDF
