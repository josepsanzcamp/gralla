...
function _pdf(hash)
{
    ...
    for (var key in data.songs[hash].pages) {
        ...
        document.getElementById("pdf").append(saltos.html('<img src="' + img + '" class="img-fluid w-100 mb-3 border ' + color + '" loading="lazy" width="' + width + '" height="' + height + '" alt="' + alt + '"/>'));
    }
    document.getElementById("pdf").classList.remove("d-none");
}
...
function _search()
{
    ...
    document.querySelector("#result div:last-child .border-bottom").classList.remove("border-bottom");
